...
| `report/ReportRequest` | `/api/v2/report/{test,request}` body: `reportTemplateId (ReportTemplateId, nullable), reportTemplateConfig (JsonNode, nullable — wins over id), timezone (String), userId (String, nullable → caller — PE wire DTO types this as a bare String, unlike ReportConfig.userId which is UserId), originator (EntityId, nullable), targets, notificationTemplateId`. |
